...
class AuthController extends Controller
{
    ...
    */
    ...
    protected function create(array $data)
    {
        ...
        $user->roles()->attach(\App\Role::where('name', 'patient')->first()); // RBAC
        return $user;
    }
        ...
        if ($validator->passes()) {
            ...
            Mail::send('auth.emails.activation', $user, function($message) use ($user) {
                $message->from('hello@app.com', 'Your Application');
                    $message->to($user['email'])->subject('Site - Activation Code')
                        ->setBody('http://lsem-record.local:8080/user/activation/' . $user['link']);
            });
